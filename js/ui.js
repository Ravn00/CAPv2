...
function renderDashboard() {
  ...
  // Ventas del mes desde el array ventas
  const now = new Date();
  const mesActual = now.toLocaleString("es-CL", { month: "numeric", year: "numeric" });
  const ventasMes = ventas.filter(v => {
    const f = v.fecha ? new Date(v.fecha) : null;
    return f && f.getMonth() === now.getMonth() && f.getFullYear() === now.getFullYear();
  });
...
function renderClientes(filter) {
  ...
  if (filter) {
    ...
    items = items.filter(c => (c.nombre||"").toLowerCase().includes(q) || (c.telefono||"").includes(q) || (c.email||"").toLowerCase().includes(q));
  }
  if (!items.length) {
    list.innerHTML = `<div class="cli-empty">${filter ? "Sin resultados" : "Aún no hay clientes. Tocá + Agregar para añadir el primero."}</div>`;
    return;
  }
  items.forEach(c => {
    const card = document.createElement("div"); card.className = "cli-card";
    card.innerHTML = `<div class="cli-card-name">${escH(c.nombre||"Sin nombre")}</div>
      <div class="cli-card-detail">
        ${c.telefono ? `<span>📞 ${escH(c.telefono)}</span>` : ""}
        ${c.email ? `<span>✉ ${escH(c.email)}</span>` : ""}
        ${c.direccion ? `<span>📍 ${escH(c.direccion)}</span>` : ""}
      </div>`;
    card.onclick = () => openClienteModal(c.id);
...
function openClienteModal(id) {
  ...
  $("cli-del-btn").style.display = c ? "" : "none";
  // Show purchase history
  let histEl = document.getElementById("cli-historial");
  if (c) {
    const compras = ventas.filter(v => v.clienteId === c.id);
    if (!histEl) {
      histEl = document.createElement("div"); histEl.id = "cli-historial";
      $("cli-notas").parentNode.after(histEl);
    }
    histEl.innerHTML = `<div style="font-size:10px;color:var(--t4);text-transform:uppercase;letter-spacing:.4px;margin:8px 0 4px">Historial de compras (${compras.length})</div>` +
      (compras.length ? compras.sort((a,b)=>new Date(b.fecha)-new Date(a.fecha)).slice(0,10).map(v =>
        `<div style="display:flex;justify-content:space-between;font-size:10px;color:var(--t2);background:var(--s3);padding:4px 8px;border-radius:var(--r4);margin-bottom:3px">
          <span>${v.fecha||"-"}</span><span style="font-family:var(--font-display);font-weight:600;color:var(--gold)">$${Math.round(v.total||0).toLocaleString("es-CL")}</span>
        </div>`
      ).join("") : `<div style="font-size:10px;color:var(--t4)">Sin compras registradas</div>`);
    histEl.style.display = "";
  } else {
    if (histEl) histEl.style.display = "none";
  }
  $("cli-modal").classList.add("on");
  $("cli-nombre").focus();
}

function closeClienteModal() {
  $("cli-modal").classList.remove("on");
  _editClienteId = null;
}
...
function saveClienteFromModal() {
  ...
  if (_editClienteId) {
    const idx = clientes.findIndex(c => c.id === _editClienteId);
    if (idx > -1) {
      clientes[idx] = { ...clientes[idx], nombre, telefono, email, direccion, notas };
      saveClienteToSupabase(clientes[idx]);
    }
  } else {
    const id = "cli-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2,6);
    const cli = { id, nombre, telefono, email, direccion, notas, createdAt: now };
    clientes.push(cli);
    saveClienteToSupabase(cli);
  }
  saveClientes();
  renderClientes($("cli-search").value);
  closeClienteModal();
  toast(_editClienteId ? "Cliente actualizado" : "Cliente agregado");
}

function deleteCliente(id) {
  showConfirm("Eliminar Cliente", "¿Eliminar este cliente? Esta acción no se puede deshacer.", async () => {
...
function saveClientes() {
  try { localStorage.setItem("ap_clientes_v2", JSON.stringify(clientes)); } catch(e) {}
}
...
function renderVentas(filter) {
  const list = $("ven-list"); list.innerHTML = "";
  let items = [...ventas];
  if (filter) {
    const q = filter.toLowerCase();
    items = items.filter(v => (v.clienteNombre||"").toLowerCase().includes(q) || v.fecha?.includes(q));
  }
  if (!items.length) {
    list.innerHTML = `<div class="cli-empty">${filter ? "Sin resultados" : "Aún no hay ventas."}</div>`;
    return;
  }
  items.forEach(v => {
    const card = document.createElement("div"); card.className = "ven-card";
    const comision = v.comision || 0;
    card.innerHTML = `<div class="ven-card-hdr"><span class="ven-card-cli">${escH(v.clienteNombre||"Cliente ocasional")}</span><span class="ven-card-total">$${Math.round(v.total||0).toLocaleString("es-CL")}</span></div>
      <div class="ven-card-meta"><span>${v.fecha||"-"}</span><span>${(v.items||[]).length} parte${(v.items||[]).length!==1?"s":""}</span><span>Vendedor: ${escH(v.vendedor||"-")}</span><span>Comisión: $${comision.toLocaleString("es-CL")}</span></div>`;
    list.appendChild(card);
  });

  // Stats
  $("ven-total-count").textContent = ventas.length;
  const totalIngresos = ventas.reduce((s,v)=>s+(v.total||0),0);
  $("ven-total-ingresos").textContent = "$"+Math.round(totalIngresos).toLocaleString("es-CL");
}

function openNuevaVenta() {
  _venItems = [];
  $("ven-modal-title").textContent = "Nueva Venta";
  $("ven-cliente").innerHTML = '<option value="">— Cliente ocasional —</option>';
  clientes.forEach(c => {
    const opt = document.createElement("option"); opt.value = c.id; opt.textContent = c.nombre;
    $("ven-cliente").appendChild(opt);
  });
  $("ven-part-search").value = "";
  $("ven-part-results").innerHTML = "";
  renderVenItems();
  $("ven-modal").classList.add("on");
  $("ven-part-search").focus();
}

function closeVentaModal() {
  $("ven-modal").classList.remove("on");
  _venItems = [];
}

function renderVenItems() {
  const el = $("ven-items"); el.innerHTML = "";
  _venItems.forEach((item,i) => {
    const row = document.createElement("div"); row.className = "ven-item-row";
  ...
  const total = _venItems.reduce((s,it) => s+((it.precio||0)*(it.cantidad||1)), 0);
  $("ven-total").textContent = "Total: $"+Math.round(total).toLocaleString("es-CL");
}

function addPartToVenta(partId) {
  const p = parts.find(x => x.id === partId);
  if (!p) return;
  const existing = _venItems.findIndex(x => x.partId === partId);
  if (existing > -1) {
    _venItems[existing].cantidad = (_venItems[existing].cantidad||1) + 1;
  } else {
    _venItems.push({ partId: p.id, marca: p.marca, modelo: p.modelo, precio: p.precioVenta||0, cantidad: 1 });
  }
  renderVenItems();
  $("ven-part-search").value = "";
  $("ven-part-results").innerHTML = "";
...
function saveVentaFromModal() {
  if (!_venItems.length) { toast("Agregá al menos una parte"); return; }
  const cliSelect = $("ven-cliente");
  const clienteId = cliSelect.value || null;
  const clienteNombre = clienteId ? clientes.find(c=>c.id===clienteId)?.nombre||"" : "Cliente ocasional";
  const total = _venItems.reduce((s,it) => s+((it.precio||0)*(it.cantidad||1)), 0);
  const id = "ven-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2,6);
  const fecha = new Date().toLocaleString("es-CL");
  const venta = { id, clienteId, clienteNombre, items: [..._venItems], total, fecha, notas: "" };
  ventas.unshift(venta);
  saveVentaToSupabase(venta);
  saveVentas();
  // Audit log
  _venItems.forEach(it => {
    if (it.partId) sbLogAudit(it.partId, "sale", { ventaId: id, cliente: clienteNombre, total });
  });
  renderVentas($("ven-search").value);
  closeVentaModal();
  toast("Venta registrada");
}

function saveVentas() {
  try { localStorage.setItem("ap_ventas_v2", JSON.stringify(ventas)); } catch(e) {}
}
